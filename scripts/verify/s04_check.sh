...
VERIFY_ROOT="$(mktemp -d "${ARTIFACT_ROOT}/s04-verify-XXXXXX")"
...
fail_case() {
  ...
  local message="$2"
  local rc="${3:-1}"
  echo "s04_check[${label}]: ${message}" >&2
...
if stems:
    raise SystemExit(f"unexpected stem outputs for {case_dir.name}: {stems}")
print(f"{case_dir.name}: no stems written")
PY
}
...
assert_healthy_case() {
  ...
  python - <<'PY' "${artifact_path}" "${case_dir}" "${expected_input}" "${DEFAULT_MODEL_PATH}"
import json
from pathlib import Path
import sys
...
if payload["drop_count"] != 0:
    raise SystemExit(f"unexpected drop_count: {payload['drop_count']}")
if payload["queue_depth"] != 20:
    raise SystemExit(f"unexpected queue_depth: {payload['queue_depth']}")
if payload["source"]["kind"] != "mp3":
    raise SystemExit(f"unexpected source kind: {payload['source']['kind']}")
if payload["source"]["reference"] != expected_input:
    raise SystemExit(f"unexpected source reference: {payload['source']['reference']}")
...
print(f"{case_dir.name}: healthy artifact ok")
PY
}

assert_degraded_case() {
  local artifact_path="$1"
  local case_dir="$2"
  python - <<'PY' "${artifact_path}" "${case_dir}" "${DEFAULT_MODEL_PATH}"
import json
from pathlib import Path
import sys
...
if payload["health_state"] != "degraded":
    raise SystemExit(f"unexpected health state: {payload['health_state']}")
if "dropped 17 additional chunks" not in payload["health_reason"]:
    raise SystemExit(f"unexpected health reason: {payload['health_reason']}")
if payload["requested_model_path"] != expected_model_path:
    raise SystemExit(f"unexpected requested_model_path: {payload['requested_model_path']}")
if payload["model_path"] != expected_model_path:
...
if payload["fallback_applied"] is not False:
    raise SystemExit(f"unexpected fallback_applied: {payload['fallback_applied']}")
if payload["queue_depth"] != 3:
    raise SystemExit(f"unexpected queue_depth: {payload['queue_depth']}")
if payload["drop_count"] != 17:
    raise SystemExit(f"unexpected drop_count: {payload['drop_count']}")
for stem_name in ["vocals", "drums", "bass", "other"]:
    expected_path = str(case_dir / f"{stem_name}.wav")
    if stem_paths.get(stem_name) != expected_path:
        raise SystemExit(f"unexpected {stem_name} path: {stem_paths.get(stem_name)}")
print(f"{case_dir.name}: degraded artifact ok")
PY
}
...
for stem_name in ["vocals", "drums", "bass", "other"]:
    ...
    if stem_paths.get(stem_name) != expected_path:
        raise SystemExit(f"unexpected {stem_name} path: {stem_paths.get(stem_name)}")
print(f"{case_dir.name}: fallback artifact ok")
PY
}
...
assert_failure_case() {
  local artifact_path="$1"
  local case_dir="$2"
  local missing_input="$3"
  python - <<'PY' "${artifact_path}" "${case_dir}" "${missing_input}" "${DEFAULT_MODEL_PATH}"
import json
from pathlib import Path
import sys
...
artifact_path = Path(sys.argv[1])
case_dir = Path(sys.argv[2])
missing_input = sys.argv[3]
expected_model_path = sys.argv[4]
payload = json.loads(artifact_path.read_text(encoding="utf-8"))
if payload["status"] != "error":
    raise SystemExit(f"unexpected status: {payload['status']}")
if payload["error_stage"] != "decode_failed":
    raise SystemExit(f"unexpected error_stage: {payload['error_stage']}")
if missing_input not in payload["error_message"]:
    raise SystemExit(f"missing input path from error_message: {payload['error_message']}")
if payload["health_state"] != "degraded":
    raise SystemExit(f"unexpected health state: {payload['health_state']}")
if payload["requested_model_path"] != expected_model_path:
    raise SystemExit(f"unexpected requested_model_path: {payload['requested_model_path']}")
...
for stem_name in ["vocals", "drums", "bass", "other"]:
    expected_path = str(case_dir / f"{stem_name}.wav")
    if stem_paths.get(stem_name) != expected_path:
        raise SystemExit(f"unexpected failure stem path for {stem_name}: {stem_paths.get(stem_name)}")
print(f"{case_dir.name}: hard failure artifact ok")
PY
}
...
run_success_case() {
  ...
  assert_healthy_case "${artifact_path}" "${case_dir}" "${expected_input}"
...
run_degraded_case() {
  ...
  if [[ "${rc}" -ne 0 ]]; then
    fail_case "${label}" "expected success with degraded health but CLI exited ${rc}; artifact=${artifact_path}; stdout=${stdout_path}; stderr=${stderr_path}"
  fi
  [[ -f "${artifact_path}" ]] || fail_case "${label}" "missing runtime artifact at ${artifact_path}"
  validate_artifact "${artifact_path}" || fail_case "${label}" "schema validation failed for ${artifact_path}"
  assert_four_stems "${case_dir}"
  assert_degraded_case "${artifact_path}" "${case_dir}"
...
run_fallback_case() {
  ...
  validate_artifact "${artifact_path}" || fail_case "${label}" "schema validation failed for ${artifact_path}"
  assert_four_stems "${case_dir}"
  assert_fallback_case "${artifact_path}" "${case_dir}" "${requested_model_path}"
...
run_failure_case() {
  ...
  if [[ "${rc}" -eq 0 ]]; then
    fail_case "${label}" "expected failure but CLI exited 0; artifact=${artifact_path}; stdout=${stdout_path}; stderr=${stderr_path}"
  fi
  [[ -f "${artifact_path}" ]] || fail_case "${label}" "missing failure artifact at ${artifact_path}"
  validate_artifact "${artifact_path}" || fail_case "${label}" "schema validation failed for ${artifact_path}"
  assert_no_stems "${case_dir}"
  assert_failure_case "${artifact_path}" "${case_dir}" "${missing_input}"
...
run_failure_case \
  "hard-failure-missing-input" \
  "fixtures/audio/missing.mp3" \
...
echo "s04_check: PASS (${PASS_COUNT} checks)"
